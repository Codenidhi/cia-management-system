...
export default function Courses() {
  ...
  useEffect(() => {
    axios.get(`${API_URL}/api/courses`, { headers: getHeaders() })
      .then((res) => { if (res.data.success && res.data.data.length > 0) setCourses(res.data.data); })
      .catch(() => {});
  }, []);

  const theory    = courses.filter((c) => c.type === "Theory"    || !c.type);
  ...
  return (
    <div className="card">
      ...
      <div className="stats-grid" style={{ marginBottom: 24 }}>
        ...
        <div className="stat-card"><div className="stat-value">{theory.length}</div><div className="stat-label">Theory</div></div>
        <div className="stat-card"><div className="stat-value">{practical.length}</div><div className="stat-label">Practical</div></div>
        <div className="stat-card"><div className="stat-value">{totalCredits}</div><div className="stat-label">Total Credits</div></div>
      </div>

      <div className="table-wrapper">
        <table>
          <thead>
            <tr><th>#</th><th>Course Code</th><th>Course Name</th><th style={{ textAlign: "center" }}>Semester</th><th style={{ textAlign: "center" }}>Credits</th><th style={{ textAlign: "center" }}>Type</th></tr>
          </thead>
          <tbody>
            {courses.map((c, i) => (
              <tr key={c.course_code || c.code || i}>
                <td style={{ color: "#999" }}>{i + 1}</td>
                <td><strong style={{ color: "#800000" }}>{c.course_code || c.code}</strong></td>
                <td style={{ fontWeight: 500 }}>{c.course_name || c.name}</td>
                <td style={{ textAlign: "center" }}>{c.semester}</td>
                <td style={{ textAlign: "center" }}><strong>{c.credits}</strong></td>
                <td style={{ textAlign: "center" }}>
                  <span className={c.type === "Theory" ? "badge-theory" : "badge-practical"}>{c.type}</span>
                </td>
              </tr>
            ))}
          </tbody>
