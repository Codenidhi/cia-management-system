...
export default function CIAMarks() {
  ...
  useEffect(() => {
    ...
    axios.get(`${API_URL}/cia-components`, { headers: getHeaders() })
      .then((r) => { if (r.data.success) setCiaComponents(r.data.data || []); })
      .catch(() => {});
  }, []);
  ...
  const handleSave = async () => {
    ...
    try {
      await Promise.all(
        entries.map((e) =>
          axios.post(`${API_URL}/cia-marks/add`, {
  ...
  return (
    <div className="card">
      <div className="card-header">
        ...
        {user?.name && <span style={{ fontSize: 13, color: "#666" }}>Faculty: <strong>{user.name}</strong></span>}
      </div>

      {error && (
        <div style={{ background: "#fff0f0", border: "1px solid #ffcccc", borderRadius: 8, padding: "10px 16px", marginBottom: 16, color: "#cc0000" }}>
          {error}
        </div>
      )}
      ...
      <div style={{ display: "flex", gap: 16, marginBottom: 20, flexWrap: "wrap" }}>
        <div style={{ flex: 1, minWidth: 200 }}>
          <label style={{ display: "block", fontSize: 13, color: "#555", marginBottom: 6, fontWeight: 600 }}>Select Course</label>
          <select value={selectedCourse} onChange={handleCourseChange}
            style={{ width: "100%", padding: "8px 12px", borderRadius: 8, border: "1.5px solid #ddd", fontSize: 14 }}>
            <option value="">-- Choose course --</option>
            {courses.length === 0 && <option disabled>No courses assigned to you</option>}
            {courses.map((c) => (
              <option key={c.id} value={c.id}>
                {c.name || c.course_name} {c.programme_name ? `(${c.programme_name})` : ""}
              </option>
            ))}
          </select>
        </div>
        ...
        <div style={{ flex: 1, minWidth: 200 }}>
          <label style={{ display: "block", fontSize: 13, color: "#555", marginBottom: 6, fontWeight: 600 }}>CIA Component</label>
          <select value={selectedCIA} onChange={(e) => setSelectedCIA(e.target.value)}
            style={{ width: "100%", padding: "8px 12px", borderRadius: 8, border: "1.5px solid #ddd", fontSize: 14 }}>
            <option value="">-- Choose CIA --</option>
            {ciaComponents.map((c) => (
              <option key={c.id} value={c.id}>
                {c.type || c.cia_type || `CIA-${c.id}`} (Max: {c.max_marks})
              </option>
            ))}
          </select>
        </div>
      ...
      {loading ? (
        ...
        <div style={{ textAlign: "center", padding: 40, color: "#888" }}>No students found for this course's programme.</div>
      ) : students.length > 0 ? (
        <>
          <div className="table-wrapper">
            <table>
              <thead>
                <tr>
                  <th>#</th><th>USN</th><th>Name</th><th>Programme</th>
                ...
                {students.map((s, i) => {
                  ...
                  return (
                    <tr key={s.id}>
                      ...
                      <td style={{ textAlign: "center" }}>
                        {pass === null ? <span style={{ color: "#bbb", fontSize: 12 }}>—</span>
                          : pass
                          ? <span style={{ background: "#e6f4ea", color: "#2e7d32", padding: "2px 10px", borderRadius: 12, fontSize: 12 }}>Pass</span>
                          : <span style={{ background: "#fce8e8", color: "#c62828", padding: "2px 10px", borderRadius: 12, fontSize: 12 }}>Fail</span>
                        }
                      </td>
                    </tr>
                  );
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 16, marginTop: 20 }}>
            <button onClick={handleSave} disabled={saving || !selectedCIA}
              style={{ background: saving || !selectedCIA ? "#ccc" : "#800000", color: "white",
                       border: "none", borderRadius: 8, padding: "10px 28px",
                       cursor: saving || !selectedCIA ? "not-allowed" : "pointer", fontSize: 14, fontWeight: 600 }}>
        ...
        <div style={{ textAlign: "center", padding: 60, color: "#bbb" }}>← Select a course to load students</div>
      )}
    </div>
  );
}