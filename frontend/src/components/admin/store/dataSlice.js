...
// Unwrap { success, data: [...] } or bare []
const unwrap = (res) => {
  const d = res.data;
  if (Array.isArray(d))       return d;
  if (Array.isArray(d?.data)) return d.data;
  if (d?.data)                return d.data;   // single object (add/update response)
  return d;
};

const make = (type, endpoint) => ({
  fetchAll: createAsyncThunk(`data/${type}/fetchAll`, async (_, { rejectWithValue }) => {
    try   { return unwrap(await axios.get(`${API}/${endpoint}`, { headers: getHeaders() })); }
  ...
  add: createAsyncThunk(`data/${type}/add`, async (body, { rejectWithValue }) => {
    try   { return unwrap(await axios.post(`${API}/${endpoint}`, body, { headers: getHeaders() })); }
    catch (e) { return rejectWithValue(e.response?.data?.message || e.message); }
  }),
  update: createAsyncThunk(`data/${type}/update`, async ({ id, data }, { rejectWithValue }) => {
    try   { return unwrap(await axios.put(`${API}/${endpoint}/${id}`, data, { headers: getHeaders() })); }
    catch (e) { return rejectWithValue(e.response?.data?.message || e.message); }
  }),
  del: createAsyncThunk(`data/${type}/delete`, async (id, { rejectWithValue }) => {
    try   { await axios.delete(`${API}/${endpoint}/${id}`, { headers: getHeaders() }); return id; }
    catch (e) { return rejectWithValue(e.response?.data?.message || e.message); }
  }),
});
...
// Named exports — match AdminDashboard.js imports exactly
...
const dataSlice = createSlice({
  ...
  extraReducers: (builder) => {
    ...
    builder.addCase(cia.del.fulfilled, (s, a) => {
      s.ciaComponents = s.ciaComponents.filter(x => x.cia_id !== a.payload);
    });
