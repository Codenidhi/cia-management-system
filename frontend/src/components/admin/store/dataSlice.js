...
export const updateStudent      = stud.update;
export const deleteStudent      = stud.del;
export const addFaculty         = fac.add;
export const updateFaculty      = fac.update;
...
const dataSlice = createSlice({
  ...
  extraReducers: (builder) => {
    ...
    builder.addCase(dept.del.fulfilled, (s, a) => {
      s.departments = s.departments.filter(d => d.department_id !== a.payload);
    });
    ...
    builder.addCase(prog.del.fulfilled, (s, a) => {
      s.programmes = s.programmes.filter(p => p.programme_id !== a.payload);
    });

    // ── STUDENTS ─────────────────────────────────────────────────
    builder.addCase(stud.fetchAll.fulfilled, (s, a) => { s.students = a.payload || []; });
    builder.addCase(stud.add.fulfilled, (s, a) => {
      if (a.payload) s.students.push(a.payload);
    });
    builder.addCase(stud.del.fulfilled, (s, a) => {
      // Backend returns the numeric id — filter by student_id
      s.students = s.students.filter(x => x.student_id !== a.payload);
    });
    ...
    builder.addCase(fac.del.fulfilled, (s, a) => {
      s.faculty = s.faculty.filter(x => x.faculty_id !== a.payload);
    });
    ...
    builder.addCase(course.del.fulfilled, (s, a) => {
      // courses use plain `id` not `course_id`
      s.courses = s.courses.filter(x => x.id !== a.payload && x.course_id !== a.payload);
    });
    ...
    builder.addCase(cia.update.fulfilled,   (s, a) => {
      if (!a.payload) return;
      const i = s.ciaComponents.findIndex(x => x.cia_id === a.payload.cia_id || x.id === a.payload.id);
      if (i !== -1) s.ciaComponents[i] = a.payload; else s.ciaComponents.push(a.payload);
    });
    builder.addCase(cia.del.fulfilled, (s, a) => {
      s.ciaComponents = s.ciaComponents.filter(x => x.cia_id !== a.payload && x.id !== a.payload);
    });
    ...
    builder.addCase(marks.update.fulfilled,   (s, a) => {
      if (!a.payload) return;
      const i = s.marks.findIndex(x => x.marks_id === a.payload.marks_id || x.id === a.payload.id);
